...
## [Unreleased] — Duplicate guest detection & merge

### Added

- **Open RSVP now asks before creating a duplicate** ([`0012_open_rsvp_confirm.sql`](supabase/migrations/0012_open_rsvp_confirm.sql)) — when the name a guest types closely matches an existing guest, `register_open_rsvp` returns `{needs_confirm, candidates}` and writes nothing; the form asks *"Is this you?"* and offers up to 3 names. Picking one reuses that guest; **No — continue as "…"** registers as typed. The prompt is a notice, never an error — it can't block a genuine new guest. Exact matches still resolve silently, as before.
- **The disclosure this adds is bounded by four controls**, all load-bearing: matching runs only *after* the PIN verifies (a wrong PIN still burns a rate-limit attempt); the 0.55 threshold means short probes match nothing, so a surname can't be used to fish; at most 3 candidates, projecting only `id` and `name`; and the existing 20-wrong-PINs/15-min lockout is unchanged. A confirmed id is **re-validated server-side** against the typed name, so a client can't claim a guest id it was never offered — a confirmation that no longer holds is refused rather than falling through to an insert.
- `registerResultKind` / `registerCandidates` in `src/lib/openRsvp.js` — `needs_confirm` carries no token but is *not* an error, so the page must branch on it before the token check; keeping that in a tested pure function rather than inline shape-sniffing. New `rsvp.dupe.*` and `rsvp.err.confirmFailed` keys across all six locales.
