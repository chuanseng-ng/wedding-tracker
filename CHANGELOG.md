...
- **A guest merge could email the guest after all — depending on when you applied `0007`.** `0007_email_automation.sql` is optional and is applied *out of numeric order* (the User Guide tells you to run it only once email is configured), but it defined `notify_rsvp_status_change` **without** the `app.suppress_rsvp_email` guard, while `0011_guest_dedupe.sql` defined the same function **with** it. Following the documented order — `0001`–`0006`, `0008`–`0012`, then `0007` — therefore left the *unguarded* body installed, silently undoing the previous entry's fix: every `merge_guests` call that moved the canonical guest from pending to confirmed emailed that guest a confirmation (and an `.ics`) they never triggered, with no error anywhere. `0007` now carries the guard as the **single** definition of the function, so it is safe to apply at any point. Existing deployments get the fix by re-pushing (see §1a). `supabase/tests/guest_merge_verification.sql` gained a BLOCK 6 that asserts the guard is present in the *installed* function, so the duplicate cannot come back.

### Changed

- **Migrations consolidated (round 4)** — the three feature migrations `0010_weddings_helper_read_hardening`, `0011_guest_dedupe` and `0012_couple_name_order` were folded back into the domain files, so every object again appears exactly once, in its final form (363 duplicated lines removed; 12 files → 10). Six objects had two definitions each and now have one: `get_wedding_config` and `upsert_wedding_config` (final forms in `0004_smart_rsvp.sql`), `get_public_wedding` and the `weddings.name_order` column (`0003_weddings_page.sql`), `get_public_runsheet` (`0006_planning_features.sql`), `notify_rsvp_status_change` (`0007_email_automation.sql`), plus the couple-only `weddings_select` policy (`0003`) and the helper-safe `get_wedding_floorplans` projection (`0005_roles_security.sql`). Guest dedupe stays its own file — renumbered `0011` → **`0010_guest_dedupe.sql`** — because it depends on `guests.self_registered` from `0008` and so cannot move earlier. Every surviving function body is byte-identical to its previous final version, with two deliberate comment-only exceptions (both show up as `pg_dump` hunks and neither changes behaviour): the in-body comment of `notify_rsvp_status_change` now points at `0010_guest_dedupe.sql` instead of `0011`, and the `normalize_guest_name` catalog comment is corrected — see below. **Existing CLI deployments:** run the one-time `schema_migrations` cleanup in `docs/USER_GUIDE.md` §1a (delete versions `0001`–`0012` — the surviving domain files gained content but kept their version numbers, so they must replay) before the next `supabase db push`.
- **Two stale comments corrected.** `0011`'s header claimed `normalize_guest_name()` was "also used by 0012" (it never was — only the dedupe migration and its test script reference it), and its catalog comment claimed the function was `IMMUTABLE` so `guests_name_norm_idx` "can use it" — no such index exists, and the adjacent comment explains that a functional index was *deliberately* not created.
...
- **"Name order" setting in Wedding Setup** (`0012_couple_name_order.sql`, since folded into `0003`/`0004`/`0006` by the round-4 consolidation) — the couple picks whether the bride's or the groom's name reads first, with a live preview under the dropdown. Stored as `weddings.name_order` (`'bride_first'` default, `'groom_first'`, CHECK-constrained), so every place that already rendered bride-first is unchanged on upgrade. The two pre-existing groom-first spots do move to bride-first, neither of them visibly: `defaultSlug()` only ever *suggests* a slug while that field is still empty (saved wedding-page URLs are never rewritten), and `wedding.inviteTag` is unreferenced. The order applies everywhere the pair is shown: public wedding page and runsheet, RSVP form, confirmation + reminder emails, the `.ics` calendar invite, Wishes Wrapped, CSV export filenames and the browser tab title.
