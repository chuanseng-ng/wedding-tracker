...
| [`0012_open_rsvp_confirm.sql`](../supabase/migrations/0012_open_rsvp_confirm.sql) | **Open-RSVP duplicate prevention**: recreates `register_open_rsvp` so a near-matching name returns `{needs_confirm, candidates}` instead of silently creating a duplicate — requires `0011` for `normalize_guest_name` |
