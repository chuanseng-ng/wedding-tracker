...
- **Guest photowall** — opt-in section on the wedding page where guests upload their own photos (PIN-gated, downscaled and EXIF-stripped in the browser). Photos appear live in a masonry wall; hide or delete any of them from the admin **Photowall** tab. Files are stored in Cloudflare R2 or Vercel Blob (`PHOTO_STORAGE_PROVIDER` — see `.env.example`), not in Supabase
