...
- **RSVP QR codes** — show any RSVP link as a scannable QR code: a generic `/rsvp` QR for the group chat or a slide, a per-guest QR of their personalized pre-filled link, downloadable as PNG, and a printable **QR sheet** with one card per guest to slip into physical invitations
...
**Sharing on paper:** the RSVP tab can also render these links as QR codes — the generic link, a per-guest personalized `?token=` QR, or a printable sheet of every guest's QR card — for physical invitations (see the User Guide).
